...
if cuda_toolkit_available():
    ...
    if os.path.exists(os.path.join(build_dir, "gstex_cuda.so")) or os.path.exists(
        os.path.join(build_dir, "gstex_cuda.lib")
    ):
        ...
        with Console().status(
            "[bold yellow]gstex_cuda: Setting up CUDA (This may take a few minutes the first time)",
    ...
    Console().print(
        "[yellow]gstex_cuda: No CUDA toolkit found. gstex_cuda will be disabled.[/yellow]"
    )
